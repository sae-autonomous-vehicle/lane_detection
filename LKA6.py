...
import time
import cv2
from LaneDetectCV6 import LaneDetect
import numpy as np
...
class AutoPilot():
    ...
    def laneCentering(self, offset):
        ...
        return self.steer
    def dynamicSpeed(self, fps, lanes_detected):

        if not lanes_detected:
            return 10  # safe crawl mode

        # Clamp FPS range to avoid instability
        fps = max(10, min(fps, 60))

        # Map FPS linearly to speed
        speed = int(np.interp(fps, [10, 60], [70, self.MAX_SPEED]))
        return speed
...
if __name__ == '__main__':
    ...
    while True:
        ...
        fps = lane_pred.fps # fallback fps
        
        # Check lane detection status
        lanes_detected = not (math.isinf(curvature) or curvature == 1e10 or (curvature == 0 and lane_offset == -1))
 
        # Compute dynamic speed
        speed = pilot.dynamicSpeed(fps, lanes_detected)
        steer = pilot.steerController(curvature, lane_offset)

        # data.send_data(steer,speed)
        print(f'curv: {curvature}, offset:{lane_offset}, steer:{steer}, speed:{speed}, fps:{fps}')
